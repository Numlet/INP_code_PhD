...
lons=sample_cube.coord('grid_longitude').points[250:]-180
...
with writer.saving(fig,'/nfs/a201/eejvt/SO_VIDEOS/2D_141116'+sample_cube.var_name+'_'+names+".mp4", 200):
    for it in range(len(sample_cube.coord('time').points)-1):
        try:
            first.append(cube_list[0][:,:,250:].extract(iris.Constraint(time=sample_cube.coord('time').points[it])).data.mean())
            second.append(cube_list[1][:,:,250:].extract(iris.Constraint(time=sample_cube.coord('time').points[it])).data.mean())
            third.append(cube_list[2][:,:,250:].extract(iris.Constraint(time=sample_cube.coord('time').points[it])).data.mean())
        except:
#            first.append(np.nan)
...
#            third.append(np.nan)
        ...
        for ir in range(n_runs):
            ...
            sub_plots.append(x)
            try:
                plot_cube=cube_list[ir][:,:,250:].extract(iris.Constraint(time=sample_cube.coord('time').points[it])).data
            except:
                plot_cube=cube_list[ir][:,:,250:].extract(iris.Constraint(time=sample_cube.coord('time').points[it-1])).data
#                a='asfdsasd'
