...
def osullivan_parametrization(T):
    #T in K 
    return np.exp(2.974e-3*T**2-2.16*T+366.3)#cm**2)
...
def calculate_INP_osullivan_ext_mean_area_fitted(T,dust_modes=[2,3]):
    std=s.sigma[:]
    #T=258
    ...
    rmean=s.rbardry[:,:,:,:,:]*1e2
    ns=osullivan_parametrization(T)
...
'''
def calculate_INP_osullivan_ext(T):
    dust_modes=[2,3]#,5,6]
    ...
    return INP
'''
INP_osullivan_ext=np.zeros((38, 31, 64, 128, 12))
for t in range(38):
    print t
    INP_osullivan_ext[t,:,:,:,:]=calculate_INP_osullivan_ext_mean_area_fitted(-t+273.15).sum(axis=0)

np.save('/nfs/a201/eejvt/INP_osullivan_ext_alltemps',INP_osullivan_ext)
